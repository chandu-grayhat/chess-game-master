...
class Pawn(ChessPiece):
        ...
        def get_moves(self, board):
            ...
            if x == end_row:
                ...
                promote_to = Queen
                # Show a dialog box to the player to choose the promotion piece
                piece_options = [('Queen', Queen), ('Rook', Rook), ('Bishop', Bishop), ('Knight', Knight)]
                promote_to = self.choose_promotion_piece(piece_options)
                moves = [(end_row, self.y, promote_to(self.color))]
            return moves

        def choose_promotion_piece(self, piece_options):
            """Shows a dialog box to the player to choose the promotion piece"""
            print('Choose a piece to promote to:')
            for index, (piece_name, _) in enumerate(piece_options):
                print(f'{index + 1}. {piece_name}')
            while True:
                try:
                    choice = int(input('Enter your choice: '))
                    if choice < 1 or choice > len(piece_options):
                        raise ValueError
                    return piece_options[choice - 1][1]
                except ValueError:
                    print('Invalid input. Please enter a number between 1 and', len(piece_options))
